...
import com.buyden.customer.config.KafkaProperties;
...
class CustomerKafkaResourceIT {
    ...
    void setup() {
        ...
        CustomerKafkaResource kafkaResource = new CustomerKafkaResource(kafkaProperties);
    ...
    void producesMessages() throws Exception {
        restMockMvc.perform(post("/api/customer-kafka/publish/topic-produce?message=value-produce"))
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc.perform(get("/api/customer-kafka/consume?topic=topic-consume"))
            .andExpect(status().isOk())
